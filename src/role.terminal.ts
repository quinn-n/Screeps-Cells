...
function run(terminal: StructureTerminal)
{
    ...
    for (const resourceString in TERM_CFG) {
        ...
        if (!RESOURCE_CFG) {
            throw Error(`Resource config for ${resource} in room ${terminal.room.name} is undefined`);
        }

        //Attempt to sell resources for min_price if there's more in store than the max
        if (RESOURCE_CFG.sell !== undefined && terminal.store[resource] > RESOURCE_CFG.sell.max) {
            const gameOrders = Game.market.getAllOrders({type: ORDER_BUY, resourceType: resource});
            const orders = _.filter(gameOrders, (order: Order) => order.price >= RESOURCE_CFG.sell.min_price);
            if (orders.length) {
                sortByPrice(orders);
                const ord = orders[0];
                ...
                if (resource === RESOURCE_ENERGY) {
                    const energy_cost = Game.market.calcTransactionCost(amount, terminal.room.name, ord.roomName);
                    if (terminal.store[resource] - RESOURCE_CFG.sell.min < amount + energy_cost) {
                        amount = (terminal.store[resource] - RESOURCE_CFG.sell.min) - energy_cost;
                    }
                }
                console.log(`Terminal in room ${terminal.room.name} attempting to sell ${amount} ${resource}`);
