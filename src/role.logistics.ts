...
function run(creep: Creep) {
	...
	for (const resourceString in logisConfig.TRANSFERS) {
		...
		//Withdraw resource
			...
			const structure = getClosest(
				creep,
				_.filter(
					fromStructures,
					(structure: StorageStructure) => structure.store[resource] > 0,
				) as StorageStructure[],
			);
...
function getToStructures(creep: Creep, resource: ResourceConstant) {
	...
	if (resCfg === undefined) {
		console.warn(
			`Got unconfigured resource ${resource} in room ${creep.room.name}`,
		);
		return [];
	}

	let toStructures: StorageStructure[] = _.filter(
		creep.room.find(FIND_MY_STRUCTURES),
		(structure: Structure) => resCfg.TO.includes(structure.structureType),
	);
	if (!resCfg.AMOUNT) {
		toStructures = _.filter(
			toStructures,
		...
		toStructures = _.filter(
			toStructures,
			(structure: StorageStructure) =>
				structure.store[resource] < resCfg.AMOUNT,
		);
	}
	return toStructures;
}
...
function getFromStructures(creep: Creep, resource: ResourceConstant) {
	...
	const fromStructures = _.filter(
		creep.room.find(FIND_MY_STRUCTURES),
		(structure: StorageStructure) =>
			resCfg.FROM.includes(structure.structureType) &&
...
*/
function getHighestStored(
	objs: StorageStructure[],
...
*/
function getClosest<T extends StructureConstant>(
	creep: Creep,
	objs: Structure<T>[],
) {
	let closest = objs[0];
